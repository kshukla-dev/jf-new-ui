...
// Map each service to a real JF photo (services.json doesn't carry images itself)
...
// Top 3 services for the foundations / "Our services" carousel
const topServices = (services.services ?? []).slice(0, 3).map((s) => ({
  ...s,
...
const activeIndex = ref(2) // starts at Priya T. (index 2)
...
const sideBlogs = computed(() => sortedBlogs.value.slice(1, 5))
...
// Trust avatars (real testimonial photos)
const trustAvatars = [
...
// The two founders behind Jackson & Frank
const founders = [
...
<template>
    ...
    <div class="brand-strip-inner">
      ...
      <div class="marquee" aria-label="Countries we operate in">
        ...
        <div class="marquee-track" aria-hidden="false">
          <span v-for="c in countries" :key="`a-${c}`" class="brand-logo">{{ c }}</span>
        </div>
        <div class="marquee-track" aria-hidden="true">
          <span v-for="c in countries" :key="`b-${c}`" class="brand-logo">{{ c }}</span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="section container">
    <div class="foundations-head">
      <h2 class="section-title">Our <em>services</em></h2>
      <RouterLink to="/employer-of-record" class="btn-secondary">See all services</RouterLink>
    </div>
    <div class="foundations-grid">
      <RouterLink
        v-for="service in topServices"
        :key="service.title"
        :to="service.href ?? '/employer-of-record'"
        class="foundation-card"
        :style="{ backgroundImage: `url('${service.image}')` }"
      >
        <div class="foundation-card-body">
          <span class="foundation-card-eyebrow">{{ service.subtitle }}</span>
          <h3>{{ service.title }}</h3>
    ...
    <div class="container ge-inner">
      <div class="ge-top">
        ...
        <div class="ge-map-visual">
          ...
          <div class="ge-float-card card-na">
            <span class="ge-fc-img" style="background-image: url('https://images.unsplash.com/photo-1496442226666-8d4d0e62e6e9?auto=format&fit=crop&q=80&w=100');"></span>
            <div>
              <strong>North America <span class="status-dot"></span></strong>
              <span>Payroll in 48 hrs</span>
            </div>
          </div>
          <div class="ge-float-card card-ap">
            <span class="ge-fc-img" style="background-image: url('https://images.unsplash.com/photo-1525625293386-3f8f99389edd?auto=format&fit=crop&q=80&w=100');"></span>
            <div>
              <strong>Asia Pacific <span class="status-dot"></span></strong>
          ...
          <div class="ge-popular-dest">
            ...
            <ul>
              <li v-for="dest in globalDestinations" :key="dest.id">
                <span class="flag"><img :src="dest.icon" alt="" /></span> {{ dest.name }}
              </li>
            </ul>
      ...
      <div class="ge-carousel-wrapper">
        <div class="ge-carousel">
          <div class="ge-card" v-for="dest in globalDestinations" :key="dest.id">
            <div class="ge-card-img" :style="{ backgroundImage: `url('${dest.image}')` }">
              <div class="ge-tags">
                <span class="ge-tag active">EOR</span>
                <span class="ge-tag">Contractor</span>
              </div>
            </div>
        ...
        <div class="ge-carousel" aria-hidden="true">
          <div class="ge-card" v-for="dest in globalDestinations" :key="dest.id + '-copy'">
            <div class="ge-card-img" :style="{ backgroundImage: `url('${dest.image}')` }">
              <div class="ge-tags">
                <span class="ge-tag active">EOR</span>
                <span class="ge-tag">Contractor</span>
    ...
    <div class="structure-about">
      ...
      <div class="sa-right">
        <div class="sa-img-wrap">
          <img src="https://www.eqstaffingsolutions.com/wp-content/uploads/2022/05/dreamstime_m_141681202-2048x1365.jpg" alt="Jackson & Frank team" class="sa-img" />
          
          <div class="sa-stats-pill">
            <div class="sa-stat">
            ...
            <div class="sa-stat">
              ...
              <div>
                <strong>700+</strong>
                <span class="sa-stat-label">Businesses</span>
                <span class="sa-stat-sub">Trust us worldwide</span>
              </div>
            </div>
            <div class="sa-stat-divider"></div>
            <div class="sa-stat">
      ...
      <div class="resources-bottom-grid">
        <RouterLink to="/guides" class="resource-category-card rcc-blue">
          ...
          <div class="rcc-content-wrap">
            ...
            <span class="rcc-link">Explore guides <span class="arrow">→</span></span>
          </div>
        </RouterLink>
        <RouterLink to="/payroll" class="resource-category-card rcc-green">
          <div class="rcc-bg" style="background-image: url('https://images.unsplash.com/photo-1554224155-8d04cb21cd6c?auto=format&fit=crop&w=400&q=80')"></div>
          <div class="rcc-bg-fade"></div>
          <div class="rcc-content-wrap">
            <div class="rcc-icon">
              <!-- Document icon -->
            ...
            <span class="rcc-link">Explore resources <span class="arrow">→</span></span>
          </div>
        </RouterLink>
        <RouterLink to="/compliance" class="resource-category-card rcc-purple">
          <div class="rcc-bg" style="background-image: url('https://images.unsplash.com/photo-1589829085413-56de8ae18c73?auto=format&fit=crop&w=400&q=80')"></div>
          <div class="rcc-bg-fade"></div>
          <div class="rcc-content-wrap">
            <div class="rcc-icon">
              <!-- Shield icon -->
            ...
            <span class="rcc-link">Explore updates <span class="arrow">→</span></span>
          </div>
        </RouterLink>
        <RouterLink to="/immigration" class="resource-category-card rcc-orange">
          <div class="rcc-bg" style="background-image: url('https://images.unsplash.com/photo-1544644181-1484b3fdfc62?auto=format&fit=crop&w=400&q=80')"></div>
          <div class="rcc-bg-fade"></div>
          <div class="rcc-content-wrap">
            <div class="rcc-icon">
              <!-- Passport icon -->
            ...
            <span class="rcc-link">Explore insights <span class="arrow">→</span></span>
          </div>
        </RouterLink>
      </div>
  ...
  <!-- ============= STAKES (featured testimonial) ============= -->
 
 <!--
  <section class="section container">
    <div class="stakes">
      <div class="stakes-img-wrap">
        <img
          src="/services/service-page/advantags.webp"
          alt="Jackson &amp; Frank team working together"
          class="stakes-img"
        />
      </div>
      <div>
        <span class="tag">What clients say</span>
        <h2 class="section-title">
          When the stakes rise, the right partner matters <em>most</em>
        </h2>
        <p class="structure-body" style="margin-top: 20px;">
          From fast-growing startups to global enterprises, HR and operations leaders trust
          Jackson &amp; Frank to expand into new markets with compliance, speed, and peace of mind.
        </p>
        <RouterLink to="/testimonials" class="btn-secondary" style="margin-top: 28px;">
        ...
        <div class="quote-card">
          ...
          <div class="quote-author">
            ...
            <div>
              <strong>{{ featuredTestimonial.name }}</strong> · {{ featuredTestimonial.role }}
            </div>
          </div>
        </div>
      </div>
  ...
  <section class="testimonials-section">
    <div class="testimonials-head-wrap">
      ...
      <span class="testimonials-eyebrow">CLIENT TESTIMONIALS</span>
    ...
    <div class="testimonials-carousel">
      ...
      <div class="carousel-pagination">
        <button
          v-for="i in Math.min(6, totalSlides)"
          :key="i"
          class="carousel-dot"
    ...
    <div class="endorsement-row">
      <svg class="laurel-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">
      ...
      <svg class="laurel-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" style="transform: scaleX(-1);">
        <path d="M11 20A7 7 0 0 1 9.8 6.1C15.5 5 17 8.48 17 12a7 7 0 0 1-6 8Z" />
        <path d="M9 10a5 5 0 0 0 4-4" />
      </svg>
    </div>
  ...
  <section class="section container faq-section">
    <div class="faq-header">
      ...
      <p class="faq-subtitle">Quick answers about global hiring and our services</p>
    </div>

    <div class="faq-tabs-wrapper">
      <div class="faq-tabs">
        <button v-for="tab in faqTabs" :key="tab.id" 
                class="faq-tab" :class="{ active: activeTab === tab.id }"
    ...
    <div class="faq-list">
      <div v-for="(faq, index) in filteredFaqs" :key="index"
           class="faq-item" :class="{ 'is-open': openFaq === index }">
        ...
        <div class="faq-answer-wrapper" v-show="openFaq === index">
  ...
  <!-- ============= FAQ ============= -->
   <!-- <section class="section container">
      <div class="faq-head">
        <h2 class="section-title" style="font-family: var(--serif); font-weight: 400;">Global Advisory FAQ</h2>
      </div>
      <div class="faq-list">
        <div
          v-for="(item, i) in faqs.items.slice(0, 6)"
          :key="item.question"
          class="faq-item"
        ...
        >
          <button class="faq-q" @click="toggleFaq(i)">
            {{ item.question }}
            <svg class="faq-chevron" :class="{ open: openFaq === i }" width="12" height="8" viewBox="0 0 12 8" fill="none" xmlns="http://www.w3.org/2000/svg">
...
.rcc-blue .rcc-icon { background-color: #1e3a8a; }
.rcc-blue .rcc-link { color: #2563eb; }
